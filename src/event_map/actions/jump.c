...
char* action_get_argv_jump(map_action_t* action, int arg_index, rtobject_t *rtobj){
  ...
  case JUMP_SCOPE_RTOBJECT:
    ...
    if (arg_index == 1){
      ...
      if (!(target_obj = get_rtobject_from_address(action->args.int_args.arg2))){
	printf("get arg error: invalid target object\n");
	return 0;
      }

      return rtobject_get_absolute_pathname(target_obj);
  ...
  case JUMP_SCOPE_CHILD:
	...
	return 0;
      ...
      return strdup(rtobject_get_name(target_obj));
